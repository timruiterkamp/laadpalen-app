<template>
  <main ref="main" class="site-overview constrained">
    <div class="site__header">
      <h1 class="site__title">Welkom bij het prototype van <span class="visually-hidden">Recharge</span></h1>
      <svg class="logo-dark" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
      ...
      <nav>
        <ul class="unstyled-list flex">
          <li><button class="btn btn--primary" data-modal="app" @click="showModal">Ga naar de app</button></li>
          <li><button class="btn btn--primary" data-modal="dashboard" @click="showModal">Ga naar het dashboard</button></li>
        </ul>
      </nav>
    </div>
    <div class="site__content container">
      <section class="site__content-section">
        ...
        <hr class="hr">
        <p class="site-paragraph">Dit is een prototype voor een voorstel voor een nieuwe app, waarbij gebruikers meldingen kunnen doorgeven over laadpalen. Deze meldingen kunnen storingen zijn, maar bijvoorbeeld ook het bezet zijn van de laadplek.</p>
      ...
      <nav>
        <ul class="unstyled-list flex">
          <li><button class="btn btn--primary white-space-nowrap" data-modal="app" @click="showModal">Ga naar de app</button></li>
          <li><button class="btn btn--primary white-space-nowrap" data-modal="dashboard" @click="showModal">Ga naar het dashboard</button></li>
        </ul>
      </nav>
    </div>
    ...
    <Modal ref="app_modal">
      <template v-slot:header>
        <div class="modal__container">
          <p>Opmerking: de app is gemaakt om op mobiel gebruikt te worden. De beste ervaring is dan ook om deze te bekijken op een mobiel apparaat.</p>
          Voor we beginnen willen we vragen om een naam door te geven voor je app account:
        </div>
      </template>
      <template v-slot:content>
        <form class="form form--center" @submit.prevent="setAppSettings">
          ...
          <input class="form-input" required type="text" autocomplete="off" name="name" id="name__input" v-model="name" />
          <input v-if="name" class="btn btn--primary white-space-wrap" type="submit" :value="'\'' + name + '\' opslaan en door naar app'" />
    ...
    <Modal ref="dashboard_modal">
      <template v-slot:header>
        <div class="modal__container">
          Voor we beginnen willen we vragen een instantie te kiezen als beheerder voor de laadpalen:
        </div>
      </template>
      <template v-slot:content>
        <form class="form form--center" @submit.prevent="setDashboardSettings">
          ...
          <select required class="form-input from-input--select" name="stakeholder" id="stakeholder__input" v-model="stakeholder">
            <option v-for="stakeholder in stakeholders" :value="stakeholder" :key="stakeholder">{{stakeholder}}</option>
          </select>
          <input v-if="stakeholder" class="btn btn--primary" type="submit" value="Opslaan en door naar dashboard" />
...
export default {
  ...
  mounted() {
    ...
    TweenLite.to(splashscreen, .4, {y: '-75vh', opacity: 0, delay: 2})
    TweenLite.to(img, .5, {y: '60vh', delay: 2, onComplete: () => {
